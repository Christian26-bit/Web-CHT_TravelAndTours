<!DOCTYPE html>
<html lang="en">
<head>
    <base href="<?php echo $baseUrl; ?>" >
    <script>const BASE_URL = "<?php echo $baseUrl; ?>";</script>
  <meta charset="UTF-8">
  <title>CHT Travel & Tour Management - New Booking (Step 1)</title>
  <link rel="stylesheet" href="assets/css/user_style.css"><!-- base user layout -->
  <link rel="stylesheet" href="assets/css/bookings.css"><!-- booking wizard -->
</head>
<body class="user-body">
  <!-- SIDEBAR (same as user dashboard) -->
  <aside class="user-sidebar">
    <div class="sidebar-logo">
      <div class="logo-circle">CHT</div>
      <div class="logo-text">
        <span>CHT Travel & Tours</span>
        <small>User Panel</small>
      </div>
    </div>

    <nav class="sidebar-menu">
      <button class="btn-new-booking" id="sidebarNewBookingBtn">+ New Booking</button>

      <a href="..//user/dashboard" class="sidebar-item">
        <span class="sidebar-icon">🏠</span> Dashboard
      </a>


      <a href="..//user/bookings" class="sidebar-item">
        <span class="sidebar-icon">📅</span> Bookings
      </a>
      <a href="..//user/clients" class="sidebar-item">
        <span class="sidebar-icon">👤</span> Clients
      </a>
      <a href="..//user/tour_packages" class="sidebar-item">
        <span class="sidebar-icon">🧳</span> Tour Packages
      </a>
      <a href="..//user/trips" class="sidebar-item">
        <span class="sidebar-icon">📍</span> Trips
      </a>
      <a href="..//user/hotel" class="sidebar-item">
        <span class="sidebar-icon">🏨</span> Hotel
      </a>
      <a href="..//user/transportation" class="sidebar-item">
        <span class="sidebar-icon">🚐</span> Transportation
      </a>
      <a href="..//user/payments" class="sidebar-item">
        <span class="sidebar-icon">💳</span> Payments
      </a>
    </nav>

    <div class="sidebar-footer">
      <button id="userLogoutBtn" class="logout-link">⟵ Logout</button>
    </div>
  </aside>

  <!-- MAIN CONTENT -->
  <main class="user-content booking-content">
    <!-- STEP PROGRESS BAR -->
    <section class="booking-steps">
      <div class="step-item active">
        <div class="step-circle">1</div>
        <span class="step-label">Customer</span>
      </div>
      <div class="step-line"></div>

      <div class="step-item">
        <div class="step-circle">2</div>
        <span class="step-label">Package</span>
      </div>
      <div class="step-line"></div>

      <div class="step-item">
        <div class="step-circle">3</div>
        <span class="step-label">Add-ons</span>
      </div>
      <div class="step-line"></div>

      <div class="step-item">
        <div class="step-circle">4</div>
        <span class="step-label">Hotel</span>
      </div>
      <div class="step-line"></div>

      <div class="step-item">
        <div class="step-circle">5</div>
        <span class="step-label">Transport</span>
      </div>
      <div class="step-line"></div>

      <div class="step-item">
        <div class="step-circle">6</div>
        <span class="step-label">Confirm</span>
      </div>
    </section>

    <!-- MAIN TWO-COLUMN AREA -->
    <section class="booking-main-layout">
      <!-- LEFT: FORM -->
      <div class="booking-form-area">
        <h1 class="booking-title">Customer Information</h1>
        <p class="booking-subtitle">
          Search for an existing customer or enter new customer details.
        </p>

        <!-- EXISTING CUSTOMER LOOKUP -->
        <div class="card booking-lookup-card">
          <label class="field-label">Existing Customer Lookup</label>
          <div class="lookup-input-wrapper">
            <span class="lookup-icon">🔍</span>
            <input
              type="text"
              id="existingCustomerSearch"
              placeholder="Search by Name, Email or ID..."
            >
          </div>
        </div>

        <!-- NEW CUSTOMER DETAILS -->
        <div class="booking-section-header">New Customer Details</div>

        <form id="bookingStep1Form" class="booking-form">
          <!-- Row 1: Full name / Contact number -->
          <div class="row-2">
            <div class="form-group">
              <label for="custFullName">Full Customer Name</label>
              <input id="custFullName" type="text" placeholder="Enter full name" required>
            </div>
            <div class="form-group">
              <label for="custContact">Contact Number</label>
              <input id="custContact" type="text" placeholder="e.g. 0912...">
            </div>
          </div>

          <!-- Row 2: Email / Preferred destination -->
          <div class="row-2">
            <div class="form-group">
              <label for="custEmail">Email Address</label>
              <input id="custEmail" type="email" placeholder="email@example.com">
            </div>
            <div class="form-group">
              <label for="custDestination">Preferred Destination</label>
              <input id="custDestination" type="text" placeholder="Enter destination">
            </div>
          </div>

          <!-- Row 3: Travel type / Pax -->
          <div class="row-2">
            <div class="form-group">
              <label for="custTravelType">Travel Type</label>
              <input id="custTravelType" type="text" placeholder="Leisure / Business">
            </div>
            <div class="form-group">
              <label for="custPax">Number of Travellers (Pax)</label>
              <input id="custPax" type="number" min="1" value="1">
            </div>
          </div>
        </form>
      </div>

      <!-- RIGHT: BOOKING SUMMARY SIDEBAR -->
      <aside class="booking-summary">
        <div class="summary-header">
          <h3>Booking Summary</h3>
          <p class="summary-progress-label">
            <span class="summary-dot"></span>
            <span id="summaryPercent">20% Complete</span>
          </p>
        </div>

        <div class="summary-items">
          <div class="summary-item">
            <div class="summary-icon">👤</div>
            <div>
              <div class="summary-label">Customer</div>
              <div class="summary-value" id="summaryCustomer">Not set</div>
            </div>
          </div>

          <div class="summary-item">
            <div class="summary-icon">🧳</div>
            <div>
              <div class="summary-label">Package</div>
              <div class="summary-value">Not selected</div>
            </div>
          </div>

          <div class="summary-item">
            <div class="summary-icon">🏨</div>
            <div>
              <div class="summary-label">Hotel</div>
              <div class="summary-value">Not selected</div>
            </div>
          </div>

          <div class="summary-item">
            <div class="summary-icon">🚐</div>
            <div>
              <div class="summary-label">Transport</div>
              <div class="summary-value">Not selected</div>
            </div>
          </div>
        </div>

        <div class="summary-progress-circle">
          <div class="circle-outer">
            <div class="circle-inner">
              <span id="summaryStepFraction">1/5</span>
            </div>
          </div>
          <p class="summary-total-label">Estimated Total</p>
          <p class="summary-total-amount">₱0</p>
          <p class="summary-total-note">Add items to see total</p>
        </div>
      </aside>
    </section>

    <!-- FOOTER NAV (Cancel / Step indicator / Next) -->
    <footer class="booking-footer">
      <button class="btn-footer-cancel" id="bookingCancelBtn">← Cancel</button>

      <div class="booking-step-indicator">
        <span>Step 1 of 6</span>
        <div class="step-progress-bar">
          <div class="step-progress-fill"></div>
        </div>
      </div>

      <button class="btn-footer-next" id="bookingNextBtn">Next →</button>
    </footer>
  </main>

  <script src="assets/js/bookings/bookingState.js"></script>
  <script src="assets/js/bookings/bookings1.js"></script>
</body>
</html>













































